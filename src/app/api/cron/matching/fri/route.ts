...
import { runFriBatchAndListProposals } from "@/lib/matching/service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[Cron:FRI] Starting for ${weekKey}`);

    // 1. Generate Proposals (includes expire + eligibility check)
    // runFriBatchAndListProposals returns list of valid proposals for this round
    const proposals = await runFriBatchAndListProposals(weekKey);
    console.log(`[Cron:FRI] Generated ${proposals.length} proposals`);
    ...
    console.log(`[Cron:FRI] Finished in ${durationMs}ms`, stats);
