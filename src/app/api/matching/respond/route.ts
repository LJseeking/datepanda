...
export async function POST(req: NextRequest) {
    try {
        ...
        // Post-transaction: Create MatchRoom + Conversation if Mutual
        let conversationId: string | null = null;
        if (result.isMutual) {
            const { userAId, userBId } = normalizeMatchPair(result.myId || userId, result.candidateId);

            // Idempotently create MatchRoom (one per pair)
            ...
            // Call ensureConversationForPair (idempotent, TalkJS kept for reference)
            try {
                const conv = await ensureConversationForPair(result.myId || userId, result.candidateId, result.weekKey);
                conversationId = conv.conversationId;
            } catch (e) {
                console.warn("[MatchRespond] TalkJS conversation skipped:", e);
            }
        }
